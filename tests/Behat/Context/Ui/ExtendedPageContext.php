...
namespace Tests\PTS\SyliusReferralPlugin\Behat\Context\Ui;
...
use Tests\PTS\SyliusReferralPlugin\Behat\Page\JQueryHelper;
...
class ExtendedPageContext extends MinkContext implements Context
{
    ...
    /**
     * @Then I see :link as a referral link
     * @param $link
     */
    public function iSeeReferralLink($link)
    {
        $value = $this->getSession()->getDriver()->getClient()
            ->getCrawler()->filter('input#referralLink')->attr('value');
        Assert::assertSame($value, $link);
    }
